...
/**
 * Missing dependency resolver logic
 */
export class ServiceProvider {
  ...
  public RegisterService<TService>(
    serviceType: string,
    registerOptions?: ServiceOptions
  ) {
  ...
  public GetService<TService>(serviceType: new () => TService) {
    ...
    if (!registration)
      throw Error(
        `ServiceProvider has not registered service called: ${typeName}`
      );
    ...
    switch (registration.mode) {
      case ServiceMode.Singleton:
        ...
        const newServiceInstance = registration.builder
  ...
  public static GetScoped<TServiceService>(
    serviceType: new () => TServiceService,
    builder?: () => TServiceService
  ) {
